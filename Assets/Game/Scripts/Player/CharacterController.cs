...
namespace GameJam.Control
{
    public class CharacterController : MonoBehaviour
    {
        ...
        private float canMoveDisbaleMaxTime = 1f;
        ...
        public UnityEvent OnCrouchExitEvent;
        ...
        private void Awake()
        {
            ...
            if (OnCrouchExitEvent == null)
                OnCrouchExitEvent = new UnityEvent();
        ...
        private void Update()
        {
            if (!canMove)
            {
                ...
                if (canMoveTimer > canMoveDisbaleMaxTime)
                {
                    canMove = true;
                    canMoveTimer = 0;
                }
        ...
        {
            if (!crouch) OnCrouchExitEvent.Invoke();
            if (canMove && !crouch)
            {
        ...
        public void ApplyDamage(float damage, Vector3 position, float force)
        {
            if (!invincible)
            {
                animator.SetBool("Hit", true);
